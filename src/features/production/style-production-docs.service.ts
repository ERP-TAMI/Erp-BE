...
export class StyleProductionDocsService {
  ...
  async update(
    ...
    if (dto.section1ImageUrl !== undefined)
      doc.section1ImageUrl = dto.section1ImageUrl ? dto.section1ImageUrl.trim() : null;
  ...
  async exportExcel(styleId: string): Promise<Buffer> {
    ...
    let docs = await this.prodDocRepo.find({
      where: { styleId },
      order: { createdAt: 'DESC' },
    });
    let doc = docs[0];
    if (!doc) {
      ...
      const freshDoc = await this.prodDocRepo.findOne({ where: { id: created.id } });
      doc = freshDoc || (created as any);
    }

    const WorkbookClass = (ExcelJS as any).Workbook || (ExcelJS as any).default?.Workbook || (ExcelJS as any).default;
    ...
    for (const sr of sizeRows) {
      ...
      if (imgUrl && typeof imgUrl === 'string' && !sizeImages.includes(imgUrl)) {
