...
CREATE TABLE fit_bom_lines (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), style_id uuid NOT NULL REFERENCES styles(id) ON DELETE CASCADE, material_id uuid REFERENCES materials(id) ON DELETE RESTRICT, material_name_snapshot varchar(255) NOT NULL, material_group_snapshot varchar(100), unit_snapshot varchar(50), material_group_id uuid REFERENCES material_groups(id) ON DELETE RESTRICT, unit_id uuid REFERENCES units(id) ON DELETE RESTRICT, consumption numeric(18,6) NOT NULL CHECK(consumption>=0), note text, order_index integer NOT NULL CHECK(order_index>=0), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), CONSTRAINT uq_fit_bom_line_order UNIQUE(style_id,order_index));
CREATE INDEX idx_fit_bom_lines_style_id ON fit_bom_lines(style_id);
