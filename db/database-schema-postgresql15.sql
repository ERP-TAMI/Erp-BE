...
CREATE TABLE purchase_order_product_documents (product_id uuid NOT NULL REFERENCES purchase_order_products(id) ON DELETE CASCADE, document_id uuid NOT NULL REFERENCES documents(id) ON DELETE RESTRICT, source_style_document_id uuid REFERENCES documents(id) ON DELETE SET NULL, source_po_document boolean NOT NULL DEFAULT false, purpose document_purpose NOT NULL DEFAULT 'other', linked_by uuid REFERENCES users(id) ON DELETE SET NULL, linked_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(product_id,document_id));
CREATE TABLE purchase_order_product_colors (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), product_id uuid NOT NULL REFERENCES purchase_order_products(id) ON DELETE CASCADE, color_name varchar(100) NOT NULL, order_index integer NOT NULL DEFAULT 0 CHECK(order_index>=0), CONSTRAINT uq_product_color UNIQUE(product_id,color_name));
